...
module bori(){
	translate([46,-89.5,0])rotate(30) import("stl/3DR_Bot_Mot.stl");
}
//bori();
...
include <nema.scad>
include <3DR_Bed_Sup.scad>
//lCov=160;
//Uncomment
bot_mot();
echo(vgx*cos(30),vy,3*vgx*sin(30),vx);
//translate([vx,vy,0]) rotate(-120) bot_mot();
nemL=60;
/*difference(){
	bot_mot();
...
module motor_guide(){
	...
	translate([sumV(v=vLM,i=6),-2,0]) 
		cube([sumX(vL,vAn,4)-sumV(v=vLM,i=6)-tBas,4,2*tBas]);
}
...
module bot_mot(){
	//translate([sumV(v=vLM,i=4)+30,0,30]) rotate([0,-90,0]) nema(l=nemL);
	difference(){
		bot_mot_nh();
